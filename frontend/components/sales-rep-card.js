...
export function SalesRepCard({ salesRep, viewMode }) {
  const totalDealValue = salesRep.deals.reduce(
      ...
      <Card className="h-full flex flex-col justify-between overflow-hidden hover:shadow-lg transition-shadow duration-300 border-slate-200 dark:border-slate-700">
        <CardHeader className="pb-2">
          <div className="flex justify-between items-start">
            ...
            <Badge
              variant="outline"
              className={`font-normal ${getRegionColor(salesRep.region)}`}
            >
              {salesRep.region}
            </Badge>
          </div>
                  ...
                  <ToolTipProvider key={index}>
                    <ToolTip>
                      <ToolTipTrigger asChild>
                        <div className="bg-slate-100 dark:bg-slate-800 rounded p-2 text-xs truncate">
                          {client.name}
                        </div>
                      </ToolTipTrigger>

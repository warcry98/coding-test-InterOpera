...
export function DashboardStats({ salesReps }) {
  const totalDealValue = salesReps.reduce(
    (total, rep) =>
      total + rep.deals.reduce((sum, deal) => sum + deal.value, 0),
    0,
  );

  const closedWonDeals = salesReps.reduce(
    (total, rep) =>
      total + rep.deals.filter((deal) => deal.status === "Closed Won").length,
    0,
  );

  const totalDeals = salesReps.reduce(
    (total, rep) => total + rep.deals.length,
    0,
  );

  const winRate =
    totalDeals > 0 ? Math.round((closedWonDeals / totalDeals) * 100) : 0;
